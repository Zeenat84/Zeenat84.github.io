...
gapminder %>%
  ...
  ggplot(aes(x = log(pop), y = log(gdpPercap))) +
    geom_point() +
    geom_smooth(method = "lm", alpha = 0.1, fill = "red")
...
plotData <- gapminder %>%
  group_by(country) %>% 
  filter(year == max(year, na.rm = T)) %>% 
  ggplot(data = plotData, aes(x = log(pop), y = log(gdpPercap), colour = continent)) +
    geom_point() +
    geom_smooth(method = "lm", alpha = 0.1, fill = "red") +
    geom_label(data = plotData %>% filter(gdpPercap > 35000), aes(label = country)) +
    facet_wrap(~continent, nrow = 1) +
    theme(legend.position = "none")
```

7. **Legendary**: Look up the `ggrepel` package and use `geom_label_repel()` from it instead of the `geom_label()`. (TAKE FROM HARIs lecture video at around 1 hour into the lecture)
```{r}
...
gapminder %>%
  ...
  ungroup() %>% 
  ggplot(mapping = aes(x = year, y = meangdpPercap, color = continent)) +
  geom_line() +
  geom_point(alpha = 0.5, size = 2.5, na.rm = T) +
  labs (x = "Year", y = "Mean GDP Per Capita", title = "Oceania and Europe have seen the biggest absolute gains in per capita GDP") +
  theme_minimal()
...
```{r}
ggsave(filename = "PerccapGDP.png", width = 7, height = 4)
```
...
gapminder %>%
  ...
  ggplot(mapping = aes(x = year, y = meanCountrygdp, color = incomeGroup)) +
  geom_line() 
